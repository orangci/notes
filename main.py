...
def build_index(posts_info):
    ...
    for meta, _ in posts_info:
        ...
        if meta.get("edited"):
            ...
            span.append(" (edited ")
            edited_time_tag = soup.new_tag("time", attrs={"datetime": datetime_attr_edited})
...
def build_rss(posts_info):
    ...
    ET.SubElement(channel, "description").text = "Occasional posts on anything and everything."
    for meta, html_content in posts_info:
        ...
        ET.SubElement(item, "pubDate").text = pubdate.strftime("%a, %d %b %Y %H:%M:%S +0000")
...
def main():
    ...
    posts_info.sort(key=lambda x: datetime.strptime(x[0]["date"], "%Y-%m-%d"), reverse=True)
